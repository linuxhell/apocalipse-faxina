...
enum Event{Detected(Result<Vec<VolumeInfo>,String>),Progress(f32,String,String,f32),Before(Snapshot),After(Snapshot),Output(String),Finished(String),Failed(String),Cancelled}
...
pub struct DefragState{
 ...
 pub progress:f32,pub current_drive_progress:f32,pub phase:String,pub status:String,pub output:String,pub current_drive:String,pub show_log:bool,
 rx:Option<Receiver<Event>>,cancel:Arc<AtomicBool>,
}
impl Default for DefragState{
 fn default()->Self{Self{selected_mode:0,volumes:vec![],reports:vec![],running:false,detecting:false,progress:0.0,current_drive_progress:0.0,phase:"Pronto".into(),status:"Detectando unidades…".into(),output:String::new(),current_drive:String::new(),show_log:false,rx:None,cancel:Arc::new(AtomicBool::new(false))}}
}
impl DefragState{
 ...
 pub fn poll(&mut self){
  ...
  for e in events{match e{
   ...
   Event::Progress(p,d,ph,local)=>{self.progress=p;self.current_drive=d;self.phase=ph;self.current_drive_progress=local;}
   ...
   Event::Finished(s)=>{self.running=false;self.rx=None;self.progress=100.0;self.current_drive_progress=100.0;self.phase="Concluído".into();self.status=s;}
 ...
 fn begin(&mut self){self.cancel.store(false,Ordering::Relaxed);self.running=true;self.progress=0.0;self.current_drive_progress=0.0;self.phase="Preparando".into();self.status="Operação em andamento.".into();self.output.clear();self.reports.clear();crate::diagnostics::operation_start("disks","multi_volume",json!({"selected":self.selected_count()}));}
}
...
fn run_multi(drives:Vec<String>,mode:Option<OptimizationMode>,tx:Sender<Event>,cancel:Arc<AtomicBool>){
 ...
 for(index,drive)in drives.iter().enumerate(){
  ...
  let _=tx.send(Event::Progress(base+span*0.02,drive.clone(),"Analisando antes".into(),2.0));
  ...
  let _=tx.send(Event::Progress(base+span*0.08,drive.clone(),"Análise antes concluída".into(),8.0));
  if let Some(mode)=mode{
   ...
   for(ci,args)in commands.iter().enumerate(){
    let fraction=ci as f32/n as f32;
    let start=base+span*(0.10+0.65*fraction);
    let step=span*0.65/n as f32;
    let local_start=10.0+65.0*fraction;
    let local_step=65.0/n as f32;
    let stage=args.iter().find(|arg|arg.starts_with('/')).map(String::as_str).unwrap_or("");
    let phase=format!("{} - etapa {}/{} {}",mode.label(),ci+1,n,stage);
    let _=tx.send(Event::Progress(start,drive.clone(),phase.clone(),local_start));
    match run_defrag_stream(drive,args,&tx,&cancel,start,step,local_start,local_step,&phase){
     Ok(o)=>{let _=tx.send(Event::Output(format!("=== {} ===\n{}",drive,o)));}
     Err(e)=>{let _=tx.send(Event::Failed(e));return;}
    }
   }
   let _=tx.send(Event::Progress(base+span*0.82,drive.clone(),"Medindo depois".into(),82.0));
  ...
  let _=tx.send(Event::Progress(base+span,drive.clone(),"Unidade concluída".into(),100.0));
 }
...
fn run_defrag_stream(
 ...
 let mut c=Command::new("defrag.exe");
 c.arg(drive).args(args).stdin(Stdio::null()).stdout(Stdio::piped()).stderr(Stdio::piped());
 #[cfg(windows)]
 c.creation_flags(CREATE_NO_WINDOW);
 let mut child=c.spawn().map_err(|e|format!("Falha ao iniciar defrag.exe: {e}"))?;
 let stdout=child.stdout.take().ok_or_else(||"Sem stdout do defrag.".to_string())?;
 ...
 for line in BufReader::new(stdout).lines(){
  ...
  if let Some(p)=extract_percent(&line){
   let normalized=(p/100.0).clamp(0.0,1.0);
   let _=tx.send(Event::Progress(
    start+span*normalized,
    drive.into(),
    phase.to_string(),
    (local_start+local_span*normalized).clamp(0.0,100.0),
   ));
  }
 ...
 if let Some(mut e)=child.stderr.take(){let _=e.read_to_string(&mut text);}
 if status.success(){Ok(text)}else{Err(format!("defrag.exe terminou com código {:?}.\n{text}",status.code()))}
}
...
pub fn sleep_repaint_hint()->Duration{Duration::from_millis(60)}
